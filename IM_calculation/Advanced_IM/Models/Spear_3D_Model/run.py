...
def main():
    ...
    for component in ["000", "090"]:
        ...
        create_im_csv(args.output_dir, im_name, component, component_dir, check_converge=False)
    #create a geom csv
...
def read_recorder_all(direction_dir, recorder_name, im_recorder_fname):
    ...
    im_recorder = os.path.join(direction_dir, os.path.join(recorder_name,im_recorder_fname))
...
def calculate_norm(im_name, output_dir, print_header = True):
    ...
    for recorder_name in ["disp","drift"]:
        ...
        for im_recorder in im_recorders:
            ...
            im_record_list['000'] = read_recorder_all(dir_000, recorder_name, im_recorder_fname)
            im_record_list['090'] = read_recorder_all(dir_090, recorder_name, im_recorder_fname)
            im_record_list['norm'] = []
            if len(im_record_list['000']) != len(im_record_list['090']):
                raise(ValueError(f"length of 000 and 090 does not match for {im_recorder_fname}"))
            #calulate norm
            for i in range(0,len(im_record_list['000'])):
                norm = np.sqrt((np.power(im_record_list['000'][i], 2) + np.power(im_record_list['090'][i], 2)))
...
def calculate_geom(output_dir,im_name):
    df_000 = pd.read_csv(os.path.join(output_dir,im_name + "_000" + ".csv"), dtype={"component": str})
    df_090 = pd.read_csv(os.path.join(output_dir,im_name + "_090" + ".csv"), dtype={"component": str})
    #
...
def agg_csv(output_dir, im_name, print_header=True):
    ...
    for component in ["000", "090", "geom","norm"]:
        component_csv_fname = os.path.join(output_dir, im_name + "_" + component + ".csv")
        df = df.append( pd.read_csv(component_csv_fname, dtype={"component": str}))
    df.set_index("component", inplace=True)
    #im_csvs = glob.glob(im_csv_glob)
    cols = list(df.columns)
    cols.sort()
    df.to_csv(os.path.join(output_dir, im_name + ".csv"), header=print_header, columns=cols) 
    #write a csv file
...
def create_im_csv(output_dir, im_name, component, component_dir, check_converge=True, print_header=True, remove_gravity = True):
    ...
    for im_recorder in im_recorders:
        ...
        im_gravity_recorder = os.path.join(im_gravity_dir,"gr_"+os.path.basename(im_recorder))
