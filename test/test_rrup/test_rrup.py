...
class TestPickleTesting:
    def test_horizdist(self, set_up):
        ...
        for root_path in set_up:
            with open(
                os.path.join(root_path, INPUT, function + "_loc1.P"), "rb"
            ) as load_file:
                loc1 = pickle.load(load_file)
            with open(
                os.path.join(root_path, INPUT, function + "_loc2_lat.P"), "rb"
            ) as load_file:
                loc2_lat = pickle.load(load_file)
            with open(
                os.path.join(root_path, INPUT, function + "_loc2_lon.P"), "rb"
            ) as load_file:
            ...
            with open(
                os.path.join(root_path, OUTPUT, function + "_h.P"), "rb"
            ) as load_file:
    ...
    def broken_test_readStationCoordsFile(self, set_up):
        ...
        for root_path in set_up:
            with open(
                os.path.join(root_path, INPUT, function + "_match_stations.P"), "rb"
            ) as load_file:
            ...
            with open(
                os.path.join(root_path, OUTPUT, function + "_stations.P"), "rb"
            ) as load_file:
    ...
    def test_computeSourcetoSiteDistance(self, set_up):
        ...
        for root_path in set_up:
            with open(
                os.path.join(root_path, INPUT, function + "_finite_fault.P"), "rb"
            ) as load_file:
                finite_fault = pickle.load(load_file)
            with open(
                os.path.join(root_path, INPUT, function + "_Site.P"), "rb"
            ) as load_file:
                Site = pickle.load(load_file)

            actual_r_rup, actual_r_jb, actual_r_x = rrup.computeSourcetoSiteDistance(
                finite_fault, Site
            )

            with open(
                os.path.join(root_path, OUTPUT, function + "_r_rup.P"), "rb"
            ) as load_file:
                expected_r_rup = pickle.load(load_file)
            with open(
                os.path.join(root_path, OUTPUT, function + "_r_jb.P"), "rb"
            ) as load_file:
                expected_r_jb = pickle.load(load_file)
            with open(
                os.path.join(root_path, OUTPUT, function + "_r_x.P"), "rb"
            ) as load_file:
    ...
    def test_source_to_distance(self, set_up):
        ...
        for root_path in set_up:
            with open(
                os.path.join(root_path, INPUT, function + "_packaged_data.P"), "rb"
            ) as load_file:
                packaged_data = pickle.load(load_file)

            actual_station_name, actual_station_Lat, actual_station_Lon, actual_dist = rrup.source_to_distance(
                packaged_data
            )

            with open(
                os.path.join(root_path, OUTPUT, function + "_station_name.P"), "rb"
            ) as load_file:
                expected_station_name = pickle.load(load_file)
            with open(
                os.path.join(root_path, OUTPUT, function + "_station.Lat.P"), "rb"
            ) as load_file:
                expected_station_Lat = pickle.load(load_file)
            with open(
                os.path.join(root_path, OUTPUT, function + "_station.Lon.P"), "rb"
            ) as load_file:
                expected_station_Lon = pickle.load(load_file)
            with open(
                os.path.join(root_path, OUTPUT, function + "_dist.P"), "rb"
            ) as load_file:
    ...
    def test_computeRrup(self, set_up):
        ...
        for root_path in set_up:
            ...
            srf_file = os.path.join(
                root_path, INPUT, "PangopangoF29_HYP01-10_S1244.srf"
            )
            station_file = os.path.join(root_path, INPUT, "sample_station.ll")

            actual_ret_val = rrup.computeRrup(
                station_file, srf_file, match_stations, n_processes
            )
