...
def populate_dict(fraction_dict, time, duration):
    ...
    if time not in fraction_dict.keys():
        fraction_dict[time] = duration / 3600.0
    else:
        fraction_dict[time] += duration / 3600.0
...
def calc_fraction(fraction_dict, start_time, end_time):
    ...
    if (
        diff_start >= duration
    ):  # if the job is completed within an complete hour, eg 8:00-9:00
...
def main():
    ...
    parser.add_argument("dir_path", type=str, help="abs path to *.out folder")
    parser.add_argument(
        "-f",
        "--file_pattern",
        type=str,
    ...
    ax.bar(df.index, df.values, width=15.0 / num_of_dates / 24.0 * 3, align="center")
    ...
    if args.csv_out:
        ...
        df_to_csv(
            bb_df, "datetime", "number_of_bb_jobs_running", args.out_dir, "bb_jobs.csv"
        )
